...
{\large{}Corrected version as of 2025-09-02}}
...
$\varepsilon_{shader}$ using a texture that has the same resolution
...
\[
\epsilon_{x,s}=\varepsilon_{x,shader}=w_{shader}\cdot\frac{\epsilon_{x,d}}{w}+\frac{w_{shader}}{2}=\frac{w_{shader}}{w}\cdot\left(\epsilon'_{x,d}+\Delta\mathbb{P}_{x}\right)+\frac{w_{shader}}{2}
\]

\[
\epsilon_{y,s}=\varepsilon_{y,shader}=h_{shader}\cdot\frac{\epsilon_{y,d}}{h}+\frac{h_{shader}}{2}=\frac{h_{shader}}{h}\cdot\left(\epsilon'_{y,d}+\Delta\mathbb{P}_{y}\right)+\frac{h_{shader}}{2}
\]
...
follows:
\[
f_{x}\cdot x''+c_{x}=u=\epsilon_{x,s}=\frac{w_{shader}}{w}\cdot\left(\epsilon'_{x,d}+\Delta\mathbb{P}_{x}\right)+\frac{w_{shader}}{2}=\frac{w_{shader}}{w}\cdot F\cdot x''+\frac{w_{shader}}{w}\cdot\Delta\mathbb{P}_{x}+\frac{w_{shader}}{2}
\]

Since this equation is true for any value of $x''$ it follows that
\[
F=\frac{w}{w_{shader}}\cdot f_{x}\qquad\wedge\qquad\Delta\mathbb{P}_{x}=\frac{w}{w_{shader}}\cdot\left(c_{x}-\frac{w_{shader}}{2}\right)
\]

Likewise in y direction:
\[
f_{y}\cdot y''+c_{y}=v=\epsilon_{y,s}=\frac{h_{shader}}{h}\cdot\left(\epsilon'_{y,d}+\Delta\mathbb{P}_{y}\right)+\frac{h_{shader}}{2}=\frac{h_{shader}}{h}\cdot F\cdot y''+\frac{h_{shader}}{h}\cdot\Delta\mathbb{P}_{y}+\frac{h_{shader}}{2}
\]
\[
\Rightarrow\qquad F=\frac{h}{h_{shader}}\cdot f_{y}\qquad\wedge\qquad\Delta\mathbb{P}_{y}=\frac{h}{h_{shader}}\cdot\left(c_{y}-\frac{h_{shader}}{2}\right)
\]
Note that in a previous version of this document the term $-\nicefrac{w_{shader}}{2}$
in the calculation of $\Delta\mathbb{P}_{x}$ and the term $-\nicefrac{h_{shader}}{2}$
in the calculation of $\Delta\mathbb{P}_{y}$ was missing due to the
false assumption that the OpenCV coordinates $(u\quad v)^{T}$ as
well as $(c_{x}\quad c_{y})^{T}$ have their origin in the middle
of the image, while in fact the origin is in the upper left corner!
...
\noalign{\vskip\doublerulesep}
\noalign{\vskip\doublerulesep}
projection offset: & $\Delta\mathbb{P}_{x}=\frac{w}{w_{shader}}\cdot\left(c_{x}-\frac{w_{shader}}{2}\right)\quad$ &  & $\Delta\mathbb{P}_{y}=\frac{h}{h_{shader}}\cdot\left(c_{y}-\frac{h_{shader}}{2}\right)\quad$\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\noalign{\vskip\doublerulesep}
